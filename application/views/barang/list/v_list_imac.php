<div class="panel-body">
	<table data-toggle="table" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="desc">
	    ...
	    <tr>
	        ...
	        <th data-field="nama_imac"  data-sortable="true"><b>Nama Imac</b></th>
	    ...
	    <tbody>
	    	<?php 
	    		...
	    		foreach ($listIMAC->result() as $row) {
	    			...
	    			echo "<tr>";
	    			echo "<td>".$i++."</td>";
	    			echo "<td>".$row->imac_no_asset."</td>";
	    			echo "<td>".$row->imac_no_it."</td>";
	    			echo "<td>".$row->imac_sn."</td>";
	    			echo "<td>".$row->imac_nama."</td>";
	    			echo "<td>".$row->imac_user."</td>";
	    			echo "<td>";
	    			if($row->imac_kondisi == 1){
	    				echo '<span class="label label-success">';
	    			}elseif ($row->imac_kondisi == 2){
	    				echo '<span class="label label-danger">';
	    			}else{
	    				echo '<span class="label label-info">';
	    			}
	    			echo $kondisi[$row->imac_kondisi]."</span></td>";
	    			$link_edit = site_url('/barang/assigment/'.$jenis_barang."/".$row->imac_id);
	    			$link_detail = site_url('/barang/detail/'.$jenis_barang."/".$row->imac_id);
	    			$link_delete = site_url('/barang/delete/'.$jenis_barang."/".$row->imac_id);
	    			$link_tarik = site_url('/assigment/tarik/'.$jenis_barang.'/'.$row->imac_id);
	    	?>
