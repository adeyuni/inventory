	    ...
	    <tbody>
	    	<?php 
	    		...
	    		foreach ($listCPU->result() as $row) {
	    			...
	    			echo "<tr>";
	    			echo "<td>".$i++."</td>";
	    			echo "<td>".$row->cpu_no_asset."</td>";
	    			echo "<td>".$row->cpu_no_it."</td>";
	    			echo "<td>".$row->cpu_service_tag."</td>";
	    			echo "<td>".$row->cpu_hostname."</td>";
	    			echo "<td>".$row->cpu_user."</td>";
	    			echo "<td>";
	    			if($row->cpu_kondisi == 1){
	    				echo '<span class="label label-success">';
	    			}elseif ($row->cpu_kondisi == 2){
	    				echo '<span class="label label-danger">';
	    			}else{
	    				echo '<span class="label label-info">';
	    			}
	    			echo $kondisi[$row->cpu_kondisi]."</span></td>";
	    			$link_edit = site_url('/barang/assigment/'.$jenis_barang.'/'.$row->cpu_id);
	    			$link_detail = site_url('/barang/detail/'.$jenis_barang.'/'.$row->cpu_id);
	    			$link_delete = site_url('/barang/delete/'.$jenis_barang.'/'.$row->cpu_id);
	    			$link_tarik = site_url('/assigment/tarik/'.$jenis_barang.'/'.$row->cpu_id);
	    	?>
