...
class rekap extends CI_Controller {
	...
	public function edit($id = null)
	{
		...
		foreach($this->data['detailRekap']->result() as $row){
			...
			$this->data['lokasi'] = $row->rekap_lokasi;
		...
		if($param1 == 'add'){
			...
			foreach ($data_barang as $key=>$value) {
				if($this->input->post('nama_barang['.$key.']')!=""){
						//echo $key." ".$value."<br />";
						echo $this->data['nama_barang'][$key] = $this->input->post('nama_barang['.$key.']');
						echo $this->data['harga'][$key] = $this->input->post('harga['.$key.']');
						echo $this->data['jml'][$key] = $this->input->post('jml['.$key.']');
